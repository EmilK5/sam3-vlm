...
- [~] 9.3 Per-prompt dedup feedback + distinct-prompt rule (agent/actions.py, agent/runner.py, agent/history.py, agent/policy_vlm_v3.py, citrus_orchestration_app.py, tests/test_prompt_feedback.py; + tests/test_policy_vlm_v3.py update) — implemented; suite green (293, +4).
- [~] 9.4 Ban LookROIA in the active arm + tiling for recall (config.py, agent/actions.py, agent/runner.py, agent/policy_vlm_v3.py, citrus_orchestration_app.py, tests/test_active_arm_tiling.py; + tests/test_policy_vlm_v3.py migration) — implemented; suite green (298, +5). Fixes two issues from a real run: (a) the v3 fallback called policy_heuristic -> emitted LookROIA (sub-ROI zoom, drops exemplars, finds nothing); now fallback = StopA, so the arm is strictly {refine, stop} and LookROIA can never appear (empty-graph stop guard dropped — bootstrap always senses first). (b) recall trailed the generic cascade because the arm never tiled; now refines are TILED over the whole tree ROI (cfg.refine_tiling, new QueryA.tiling field) and the bootstrap adds a tiled_seed_pass (runner bootstrap_tiled_pass) replicating the generic cascade's recall floor, so active recall >= generic by construction and the VLM can only add. Additive: QueryA.tiling default False, execute_pass/apply_nms untouched, v2 look/stop policy unchanged. Check: active trajectory = canopy_roi/leaf_map/global_pass/tiled_seed_pass then tiled refines, NO LookROIA; recall >= generic; invalid/repeat reply -> single "stopping" line.
- [~] 9.5 Refine threshold rises with each new prompt (config.py, agent/policy_vlm_v3.py, tests/test_refine_threshold.py; + test_policy_vlm_v3.py/test_runner_refine.py migration) — implemented; suite green (303). Tiling made 0.40 too loose on later passes. Raised range (default 0.40->0.50, min 0.30->0.45, max 0.70->0.85) + new refine_conf_step=0.05; the threshold FLOOR = clamp(default + n_prior_refines*step, [min,max]), VLM may go higher but not lower, missing->floor. So each new prompt is >= one step stricter. Check: active trajectory shows thresholds climbing (~0.50/0.55/0.60) and fewer clutter boxes late.
- [~] 9.6 Per-pass animation + separate leaf map (citrus_orchestration_app.py) — implemented; app compiles (py_compile). draw_sam3_view gained max_pass (nodes with found_in_pass<=k; monotone graph => belief as-of-pass-k); render_pass_frames -> gr.Gallery (scroll pass 1..N, captioned action/prompt/thr + det/new/redet + N_obs); render_leaf_map -> gr.Image (cached_leaf_boxes ROI->global on its own panel). App-only, GPU hand-verified (loads SAM3 at import -> no offline test). Check: gallery scrolls through passes, boxes accumulate as prompt/threshold change; leaf map on its own panel; nav clears both. PassStats.duplicates_rejected/post_nms now thread detection->observation->policy as n_redetected/n_detections (stashed on ctx.last_pass_stats by _execute_query, copied to y by runner._obs); the VLM sees per-prompt {n_detections,n_new,n_redetected} and MUST propose a new wording each step (repeats rejected -> fallback). Additive/backward-compatible: y gained keys (tests read keys individually, one full-dict assert updated), ActionContext gained a defaulted field, apply_nms/execute_pass untouched. Check: read one active-arm prompt — prompts_tried lists each wording with its dedup re-detection count; VLM never repeats a wording; watch it pivot away from a low-n_redetected (off-concept) prompt.
