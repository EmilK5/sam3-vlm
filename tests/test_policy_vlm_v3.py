...
def _history():
    """A confident seed pass (@0.65): 7 detections, 2 new, 5 re-detected already-known
    -- the per-prompt feedback the v3 policy distills into prompts_tried."""
    h = EpisodeHistory()
    h.append({"action": "global_pass", "region": TREE_ROI, "prompt": "green fruit", "conf": 0.65},
             {"n_new": 2, "n_redetected": 5, "n_detections": 7, "new_nodes": [],
              "totals": {"K": 2, "N_obs": 2}})
    return h
...
def test_request_carries_two_images_history_and_menu():
    ...
    # the per-prompt feedback the VLM judges a wording by (incl. the dedup re-detections)
    assert body["prompts_tried"] == [
        {"prompt": "green fruit", "conf": 0.65, "n_detections": 7, "n_new": 2, "n_redetected": 5}
    ]
...
def test_overlong_prompt_stops():
    cfg, graph, phi = _fixture()
    client = _CaptureClient('{"action": "refine", "args": {"prompt": "small round waxy green citrus fruit"}}')
    action = policy_vlm_v3.choose(phi, _history(), graph, cfg, client=client, image=FRAME)
    assert isinstance(action, StopA)                             # invalid -> stop, never LookROIA
    assert not isinstance(action, QueryA)


def test_empty_prompt_stops():
    cfg, graph, phi = _fixture()
...
def test_repeated_prompt_stops():
    # "green fruit" was the seed pass in _history(); repeating it is rejected so every
    # step tries a NEW wording. Case/whitespace-insensitive -> the arm stops.
    cfg, graph, phi = _fixture()
    client = _CaptureClient('{"action": "refine", "args": {"prompt": "Green   fruit", "threshold": 0.4}}')
    action = policy_vlm_v3.choose(phi, _history(), graph, cfg, client=client, image=FRAME)
    assert isinstance(action, StopA)
    assert not (isinstance(action, QueryA) and action.prompt.lower().split() == ["green", "fruit"])
...
def test_stop_on_empty_graph_stops():
    # The bootstrap senses before the loop, so an empty graph means the seed found
    # nothing (empty orchard) -- stopping with 0 is correct, and there is no LookROIA
    # to fall back to.
    cfg = Config()
    graph = OrchardGraph()
    graph.tree_roi = list(TREE_ROI)
    ...
    client = _CaptureClient('{"action": "stop", "args": {"estimate_name": "N_obs"}}')
    action = policy_vlm_v3.choose(phi, _history(), graph, cfg, client=client, image=FRAME)
    assert isinstance(action, StopA) and action.estimate_name == "N_obs"
...
def test_no_image_refine_response_stops():
    cfg, graph, phi = _fixture()
    ...
    assert isinstance(action, StopA)                        # no image -> refine illegal -> stop
